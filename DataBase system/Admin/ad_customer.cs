        ...
        private void buttonadd_Click(object sender, EventArgs e)
        {
            if ((!string.IsNullOrEmpty(textBoxfn.Text)) && (!string.IsNullOrEmpty(textBoxln.Text)) && (!string.IsNullOrEmpty(textBoxnic.Text)) && (!string.IsNullOrEmpty(textBoxadd.Text)) && (!string.IsNullOrEmpty(textBoxema.Text)) && (!string.IsNullOrEmpty(textBoxpn1.Text)) && (!string.IsNullOrEmpty(textBoxpn2.Text)))
            {
                using (SqlConnection Con = new SqlConnection(connectionString))
                {
                    ...
                    // Use parameterized query
                    cmd.CommandText = "INSERT INTO [customer] (f_name, l_name, nic, addressis, email, phone_no1, phone_no2) VALUES (@f_name, @l_name, @nic, @addressis, @email, @phone_no1, @phone_no2)";
                    ...
                    cmd.ExecuteNonQuery();
                    Con.Close();
                    DialogResult dialogResult = MessageBox.Show("Customer Added Successfully", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                DialogResult dialogResult = MessageBox.Show("Error, Please fill all the field boxes", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
}
